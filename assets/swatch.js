...
//                 if (typeof index === 'undefined') return;
           ...
           selectCallback(variant, selector, IdSelect, size_ops, callBackVariant, NtId, prefix, product_id, click_human, pr_incoming_mess,use_notify_me,save_js,enableHistoryState);
        });
    ...
    function selectCallback(variant, selector, IdSelect, size, callBackVariant, NtId, prefix, product_id, human,pr_incoming_mess,use_notify_me,save_js,enableHistoryState) {
       ...
       if (variant) {
          ...
          var customPrice = geckoShopify.formatMoney(variant.price),
              _onsale = $('#product'+prefix.replace('_ppr','').replace('_','')+'-' + product_id + ' .onsale');
    ...
    geckoShopify.NtproductPage = function (product_id) {
      // Stop parsing if we don't have the product json script tag when loading
      // section in the Theme Editor
      if (!$('#ProductJson-template'+product_id).html()) {
        return;
      }

        var productJson = JSON.parse($('#ProductJson-template'+product_id).html()),
          incomingJson = JSON.parse($('#ProductJson-incoming'+product_id).html()),
          IdSelect = '#product-select'+product_id,NtId = '#nt_select'+product_id+'_',selector = '#cart-form'+product_id,callBackVariant = '#callBackVariant'+product_id,prefix=''+product_id,
          i,variant,Arr_MD = [];
      ...
      //console.log(product);
         ...
         geckoShopify.Ntproduct_switch('.variations_form'+product_id,Arr_MD,productJson,selector,IdSelect,NtId,callBackVariant,prefix);
         if (nt_settings.pr_curent !== "1" || $('#cart-form_ppr .is-selected-nt').length>0) {
             $('#nt_select'+product_id+'_1 .is-selected-nt, #nt_select'+product_id+'_2 .is-selected-nt').addClass('is-selected').removeClass('is-selected-nt');
             $('#nt_select'+product_id+'_0 .is-selected-nt').click().removeClass('is-selected-nt');
             
              if (!use_vimg) return;
              $('#nt_select'+product_id+'_1 .is-selected,#nt_select'+product_id+'_2 .is-selected').removeClass('is-selected').click();
         }
    };
  ...
  //        handleTouch: false
    ...
    geckoShopify.progressbar = function (id,qty) {
        ...
        var product_id = $id.data('prid');
        var updateMeter = function (a, remaining_items,bgprocess,bgten) {
            //if(sp_nt_storage) {res = sessionStorage.getItem('qv'+ntid)}
            remaining_items =  parseInt(remaining_items);
            if(sp_nt_storage) {sessionStorage.setItem('probar'+product_id, remaining_items)}
        ...
        var total_items = parseInt($id.data('total')),
           ...
           if(sp_nt_storage) {
            var getse_re = sessionStorage.getItem('probar'+product_id);
            if (getse_re > 0) {
    ...
    geckoShopify.progressbarUpdateATC = function (el,qty) {
      ...
      if (el.data('type') != 'ATC') return;
       ...
       //if (!bl) return;
        var product_id = el.data('prid');
       //if(sp_nt_storage) { var get_count = sessionStorage.getItem('probar'+product_id)}
       ...
       //if(sp_nt_storage) {sessionStorage.setItem('probar'+product_id, vl_count)}
...
jQuery_T4NT(document).ready(function($) {
  ...
  if ( $('[data-js-callback] .nt_pr_js').length > 0 ) {
    ...
    .one('lazyincluded', function(e) {
      ...
      var product_id =  $(e.currentTarget).data('id');
      //console.log('product_id: ',product_id)
      geckoShopify.NtproductPage(product_id);
      geckoShopify.initModelViewerLibraries(product_id);
      geckoShopify.initShopifyXrLaunch(product_id);
