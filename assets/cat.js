...
//         previousScroll = currentScroll;
   ...
   geckoShopify.VariantFilterRefresh = function () {
    ...
    function update_attributes(bl,url) {
      ...
      //if ( !nt_settings.auto_hide_out_of_stock ) return;
      $ld.trigger("ld_bar_star")
         fetch('/cart/update.js', {
           body: JSON.stringify({
             attributes: geckoShopify.defineProperty({}, 'auto_hide_out_of_stock', bl)
           }),
           credentials: 'same-origin',
           method: 'POST',
           headers: {
  ...
  geckoShopify.ClearFilterStockt4 = function () {
    ...
    if (!nt_settings.show_hide_out_of_stock) return;
...
jQuery_T4NT(document).ready(function($) {
  ...
  // });
 // if (!nt_settings.auto_hide_out_of_stock || t_name != 'collection') return;
